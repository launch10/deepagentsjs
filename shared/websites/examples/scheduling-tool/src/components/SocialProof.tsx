import { Users, TrendingUp, Clock, Quote } from 'lucide-react';

export function SocialProof() {
  const stats = [
    {
      icon: Users,
      value: '2,000+',
      label: 'Distributed Teams',
    },
    {
      icon: TrendingUp,
      value: '80%',
      label: 'Less Coordination Time',
    },
    {
      icon: Clock,
      value: '15 hrs/week',
      label: 'Saved Per Team',
    },
  ];

  return (
    <section className="bg-muted py-16 md:py-20 lg:py-24">
      <div className="container mx-auto px-4 md:px-6">
        {/* Section Headline */}
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-12 md:mb-16">
          Trusted by Distributed Teams Worldwide
        </h2>

        {/* Stats Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-16 md:mb-20">
          {stats.map((stat, index) => {
            const Icon = stat.icon;
            return (
              <div
                key={index}
                className="text-center transition-all duration-300 hover:-translate-y-1"
              >
                <div className="flex justify-center mb-4">
                  <Icon className="w-10 h-10 md:w-12 md:h-12 text-primary" />
                </div>
                <div className="text-4xl md:text-5xl font-bold text-foreground mb-2">
                  {stat.value}
                </div>
                <div className="text-lg md:text-xl text-muted-foreground">
                  {stat.label}
                </div>
              </div>
            );
          })}
        </div>

        {/* Testimonial Card */}
        <div className="max-w-3xl mx-auto">
          <div className="bg-card rounded-2xl shadow-lg p-8 md:p-10 lg:p-12 transition-all duration-300 hover:shadow-xl">
            <Quote className="w-10 h-10 md:w-12 md:h-12 text-primary mb-6 opacity-50" />
            <blockquote className="text-lg md:text-xl lg:text-2xl text-card-foreground mb-6 leading-relaxed">
              "We used to waste hours every week just trying to schedule meetings across our SF, London, and Tokyo offices. Now it takes 30 seconds."
            </blockquote>
            <cite className="text-base md:text-lg text-muted-foreground not-italic font-medium">
              — Sarah Chen, Project Manager at TechCorp
            </cite>
          </div>
        </div>
      </div>
    </section>
  );
}
