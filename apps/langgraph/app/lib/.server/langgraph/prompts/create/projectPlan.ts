import { projectPlanSchema } from "@models/project/projectPlan";
import { StructuredOutputParser } from "@langchain/core/output_parsers";

/**
 * Generates the prompt for the ProjectPlanNode to brainstorm OVERALL landing page copy.
 *
 * @param initialPrompt The user's initial request.
 * @returns The formatted prompt string for the LLM.
 */
export function projectPlanPrompt(initialPrompt: string): string {
  const schema = projectPlanSchema.omit({ theme: true })
  const parser = StructuredOutputParser.fromZodSchema(schema);
  const formatInstructions = parser.getFormatInstructions();

  const prompt = `
## Task: Generate Emotionally Resonant Copy for an Entire Landing Page
...
**The 9-Step Emotional Copywriting Playbook (Apply to the WHOLE PAGE):**

Before you write, pinpoint exactly what emotional core you’re targeting (you may be targeting several):
...
### Putting It All Together: Landing Page Template

**[Hero Section - Powerful Emotional Hook]**

**[Empathy Section - Mirroring Customer’s Words & Emotions]**

**[Emotional Bridge - Transition to Solution]**

**[Solution Reveal - Product as Emotional Solution]**

**[Social Proof - Emotional Validation & Trust]**

**[Urgency - Fear of Missing Out & Scarcity]**

**[CTA - Emotionally Aligned & Action-oriented]**
