class SubscribedController < ApplicationController
  ...
  inertia_share do
    ...
    [:notice, :error, :info].each do |flash_type|
      ...
      flash_messages << if parsed.is_a?(Hash) && parsed["title"]
        {type: type_map[flash_type], title: parsed["title"], description: parsed["description"]}
      else
        {type: type_map[flash_type], message: flash[flash_type]}
      end
    end
  ...
  def credits_data
...

    {
      ...
      tier: current_account.plan.tier.display_name,
