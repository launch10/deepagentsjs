...
/**
 * Image thumbnail component for chat attachments.
...
export function ImageThumbnail({ attachment, onRemove }: ImageThumbnailProps) {
  ...
  useEffect(() => {
    // Use existing previewUrl if available, otherwise create from file
  ...
  return (
    ...
    >
      ...
      {previewUrl && isComplete && (
        <img
          src={previewUrl}
          alt={attachment.file?.name ?? "Image attachment"}
