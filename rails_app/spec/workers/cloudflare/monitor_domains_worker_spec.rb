...
RSpec.describe Cloudflare::MonitorDomainsWorker, type: :worker do
  ...
  describe "#perform" do
    context "domain request count behavior" do
      ...
      it "new data replaces old data for the same hour (not accumulates)" do
        ...
        Timecop.freeze(UTC.parse("2025-08-01 10:30:00")) do
          ...
          account_total = AccountRequestCount.find_by(account: apples_account, month: day1.beginning_of_month)
          expect(account_total.request_count).to eq(130_000) # Account total matches domain totals
        end
      end
      it "AccountRequestCounts equal the sum of all DomainRequestCounts for a account for the month" do
        ...
        apples_total = DomainRequestCount.total_for_account(apples_account, next_month_day1.beginning_of_month, next_month_day1.end_of_month)
        apples_account_count = AccountRequestCount.find_by(account: apples_account, month: next_month_day1.beginning_of_month)
        expect(apples_account_count).to be_present
        expect(apples_account_count.request_count).to eq(apples_total)
        expect(apples_account_count.request_count).to eq(1900)
      end
      ...
      it "correctly handles traffic across multiple days" do
        ...
        Timecop.freeze(UTC.parse("2025-08-01 10:00:00")) do
          subject.perform(zone_id)
          day1_total = DomainRequestCount.total_for_account(apples_account, day1, day1.end_of_day)
    ...
    context "plan limit enforcement" do
      ...
      it "does not block account when under plan limit" do
        Timecop.freeze(UTC.parse("2025-08-01 10:00:00")) do
      ...
      it "blocks account when exceeding plan limit" do
        ...
        firewall = apples_account.reload.firewall
      ...
      it "raises error when Cloudflare API returns non-success response" do
        ...
        expect(apples_account.reload.firewall).to be_inactive
      end
    end
  end
  ...
  describe "Unblocking behavior" do
    ...
    context "when account was blocked in previous month" do
      ...
      it "does not unblock account when still in same month" do
      ...
      it "unblocks account when new month starts" do
        Timecop.freeze(UTC.parse("2025-09-01 10:00:00")) do
