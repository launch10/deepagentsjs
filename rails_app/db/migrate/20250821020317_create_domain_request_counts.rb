class CreateDomainRequestCounts < ActiveRecord::Migration[8.0]
  def up
    safety_assured do 
      ...
      execute <<-SQL
        CREATE INDEX index_domain_request_counts_on_domain_id_and_created_at
        ON domain_request_counts (domain_id, created_at);

        CREATE INDEX index_domain_request_counts_on_domain_created_count
        ON domain_request_counts (domain_id, created_at, request_count);

        CREATE INDEX index_domain_request_counts_on_user_id_and_created_at
        ON domain_request_counts (user_id, created_at);

        CREATE INDEX index_domain_request_counts_on_user_domain_and_created_at
