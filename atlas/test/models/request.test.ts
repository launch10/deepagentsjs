...
describe('Request Model', () => {
  ...
  describe('findByAccountId', () => {
    it('should find request by account ID', async () => {
      ...
      const found = await requestModel.findByAccountId('1');

      expect(found).toEqual(requestData);
    });

    it('should handle numeric account IDs', async () => {
      const requestData: RequestType = {
        id: uuidv4(),
        accountId: '1',
        count: 100,
      ...
      const foundByNumber = await requestModel.findByAccountId(1 as any);
      expect(foundByNumber).toEqual(requestData);

      // Should find with string ID
      const foundByString = await requestModel.findByAccountId('1');
      expect(foundByString).toEqual(requestData);
    });

    it('should return null if not found', async () => {
      const found = await requestModel.findByAccountId('nonexistent');
      expect(found).toBeNull();
    });

    it('should prevent duplicate account IDs', async () => {
  ...
  describe('Real-world scenario: rate limiter update', () => {
    it('should handle rate limiter increments correctly', async () => {
      ...
      let existing = await requestModel.findByAccountId(accountId);
      ...
      for (let i = 1; i <= 10; i++) {
        existing = await requestModel.findByAccountId(accountId);
        expect(existing).not.toBeNull();
        
        await requestModel.set(existing!.id, { count: existing!.count + 10 });
        
        const updated = await requestModel.findByAccountId(accountId);
        expect(updated?.count).toBe(i * 10);
        expect(updated?.id).toBe(newRequest.id); // ID should not change
      }
    });

    it('should not create duplicate requests for same account', async () => {
      const accountId = '1';
      
      // Simulate what happens in rateLimiterMiddleware
      const request1 = await requestModel.findByAccountId(accountId);
      ...
      const request2 = await requestModel.findByAccountId(accountId);
      expect(request2).not.toBeNull();

      // Try to create another request with same account ID (simulating race condition)
      ...
      const finalRequest = await requestModel.findByAccountId(accountId);
  ...
  describe('KV storage verification', () => {
    it('should store correct keys in KV', async () => {
      ...
      // Check unique index
      const indexKey = 'index:request:accountId:1';
