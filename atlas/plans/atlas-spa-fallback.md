...
**Critical Issue Found**: The proposed code checked `hasFileExtension` _after_ path normalization. Since existing code at lines 99-103 already transforms `/pricing` → `pricing/index.html`, the extension check would always be true and fallback would never trigger.
