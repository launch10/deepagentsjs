...
export const projectFactory = {
  ...
  async createMany(
    count: number,
    overrides: Override<typeof projects.$inferInsert> = {}
  ) {
    ...
    const values = Array(count)
      .fill(null)
      .map(() => this.build({ ...overrides, accountId }));
    const results = await db.insert(projects).values(values).returning();
    return results;
  },

  async createWithAccount(
    projectOverrides: Override<typeof projects.$inferInsert> = {},
    accountOverrides: Override<typeof accounts.$inferInsert> = {}
  ) {
...
export const accountFactory = {
  ...
  async createMany(
    count: number,
    overrides: Override<typeof accounts.$inferInsert> = {}
  ) {
    const values = Array(count)
      .fill(null)
      .map(() => this.build(overrides));
    const results = await db.insert(accounts).values(values).returning();
    return results;
  },
};
...
export const websiteFactory = {
  ...
  async createMany(
    count: number,
    overrides: Override<typeof websites.$inferInsert> = {}
  ) {
    ...
    const values = Array(count)
      .fill(null)
      .map(() => this.build({ ...overrides, projectId, accountId }));
    const results = await db.insert(websites).values(values).returning();
    return results;
  },
  ...
  async createWithAssociations() {
    ...
    const contentStrategy = await contentStrategyFactory.create({
...
export const pageFactory = {
  ...
  async createMany(
    count: number,
    overrides: Override<typeof pages.$inferInsert> = {}
  ) {
    ...
    const results = await db.insert(pages).values(values).returning();
    return results;
  },
...
export const fileSpecFactory = {
  build: (overrides: Override<typeof fileSpecifications.$inferInsert> = {}) => {
    ...
    const base = {
      canonicalPath: `/src/components/${faker.helpers.slugify(
        faker.lorem.word()
      )}.tsx`,
  ...
  async create(
    overrides: Override<typeof fileSpecifications.$inferInsert> = {}
  ) {
    const [fileSpec] = await db
      .insert(fileSpecifications)
      .values(this.build(overrides))
      .returning();
    return fileSpec;
  },
  ...
  async seed() {
    ...
    const specs = await Promise.all(
      componentTypes.map((type) => this.create({ componentType: type }))
    );
    return specs;
  },
  ...
  async createAllSections() {
    const sections = Object.keys(FILE_SPEC_SEEDS).filter(
      (key) =>
        FILE_SPEC_SEEDS[key as keyof typeof FILE_SPEC_SEEDS].filetype ===
        "Section"
    );
    const specs = await Promise.all(
      sections.map((type) => this.create({ componentType: type }))
    );
    return specs;
  },
  ...
  async createAllLayouts() {
    ...
    const specs = await Promise.all(
      layouts.map((type) => this.create({ componentType: type }))
    );
    return specs;
  },
  ...
  async createAllPages() {
    ...
    const specs = await Promise.all(
      pages.map((type) => this.create({ componentType: type }))
    );
    return specs;
  },
  ...
  async createAllConfigs() {
    ...
    const specs = await Promise.all(
      configs.map((type) => this.create({ componentType: type }))
    );
    return specs;
  },
  ...
  async createAllStyles() {
    ...
    const specs = await Promise.all(
      styles.map((type) => this.create({ componentType: type }))
    );
    return specs;
  },
...
const purposeMap: Record<SectionTypeEnum, string> = {
  [SectionTypeEnum.Hero]:
    "Capture attention and communicate the core value proposition immediately",
  [SectionTypeEnum.Features]:
    "Showcase key capabilities and functionality to demonstrate product value",
  [SectionTypeEnum.Benefits]:
    "Translate features into meaningful outcomes for the user",
  [SectionTypeEnum.HowItWorks]:
    "Explain the process or methodology in simple, digestible steps",
  [SectionTypeEnum.Testimonials]:
    "Build trust through authentic customer success stories",
  [SectionTypeEnum.Pricing]:
    "Present pricing options clearly to facilitate decision-making",
  ...
  [SectionTypeEnum.SocialProof]:
    "Establish credibility through numbers, logos, and achievements",
  [SectionTypeEnum.Team]:
    "Humanize the brand by showcasing the people behind the product",
  [SectionTypeEnum.Custom]:
    "Deliver specialized content unique to this particular use case",
};

const contextMap: Record<SectionTypeEnum, string> = {
  [SectionTypeEnum.Hero]:
    "This is the first thing visitors see - it needs to immediately communicate value and encourage scrolling or action",
  [SectionTypeEnum.Features]:
    "After grabbing attention with the hero, we need to show concrete capabilities that solve real problems",
  [SectionTypeEnum.Benefits]:
    "Users need to understand not just what the product does, but how it improves their situation",
  [SectionTypeEnum.HowItWorks]:
    "Reducing perceived complexity helps users feel confident they can successfully use the product",
  [SectionTypeEnum.Testimonials]:
    "Real stories from real users provide social proof and overcome skepticism",
  [SectionTypeEnum.Pricing]:
    "Clear pricing information respects the user's time and helps qualify leads",
  [SectionTypeEnum.FAQ]:
    "Anticipating questions shows we understand our users and reduces support burden",
  [SectionTypeEnum.CTA]:
    "A focused call to action at key moments drives conversion",
  [SectionTypeEnum.SocialProof]:
    "Logos, numbers, and awards provide quick credibility signals",
  [SectionTypeEnum.Team]:
    "Showing the team builds trust and connection with potential customers",
  [SectionTypeEnum.Custom]:
    "This custom section addresses specific needs unique to our target audience",
};
...
export const componentOverviewFactory = {
  build: (overrides: Override<typeof componentOverviews.$inferInsert> = {}) => {
    const componentType =
      ...
      faker.helpers.arrayElement(Object.values(SectionTypeEnum));
  ...
  async create(
    overrides: Override<typeof componentOverviews.$inferInsert> = {}
  ) {
...
export const contentStrategyFactory = {
  ...
  async create(
    overrides: Override<typeof contentStrategies.$inferInsert> = {}
  ) {
  ...
  async createB2BSaaS(websiteId?: number) {
    return this.create({
      ...
      landingPageCopy:
        "Transform your business with our cutting-edge SaaS platform. Automate workflows, increase productivity, and stay ahead of the competition.",
      websiteId,
    });
  },
  ...
  async createEcommerce(websiteId?: number) {
    return this.create({
      ...
      emotionalBridge:
        "Imagine having everything you need delivered to your door",
      ...
      landingPageCopy:
        "Shop the best products from top brands. Fast shipping, easy returns, and unbeatable prices.",
      websiteId,
    });
  },
};
...
const COMPONENT_CONTENT_SEEDS = {
  Hero: {
    ...
    subheadline:
      "Join thousands of companies that have revolutionized their workflow",
    paragraphs:
      "Our platform provides cutting-edge solutions that scale with your needs.",
  ...
  Features: {
    ...
    features: [
      {
        ...
        description:
          "Real-time insights and reporting. Track performance metrics in real-time",
        visual: "BarChart icon or dashboard screenshot",
      },
      {
        name: "Team Collaboration",
        description:
          "Work together seamlessly. Share and collaborate with your team",
        visual: "Users icon or collaboration interface",
      },
      {
        name: "Automation Tools",
        description:
          "Automate repetitive tasks. Save hours every week with smart automation",
        visual: "Cog icon or automation flow diagram",
      },
      {
        name: "Custom Integrations",
        description:
          "Connect with your favorite tools. Seamless integration with 100+ apps",
    ...
    layoutDescription:
      "Two-column alternating layout with feature descriptions and visuals",
    visualStyleNotes:
      "Feature screenshots or illustrations with descriptive text",
  },
  FAQ: {
    ...
    qaPairs: [
      {
        ...
        answer:
          "We offer flexible monthly and annual plans. All plans include core features with additional capabilities in higher tiers.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "Yes, we provide 24/7 customer support via chat, email, and phone for all paid plans.",
      },
      {
        question: "Is my data secure?",
        answer:
          "We use bank-level encryption and are SOC 2 certified. Your data is always protected.",
      },
    ],
  ...
  Team: {
    ...
    layoutDescription:
      "Grid layout with team member cards including photos and bios",
    visualStyleNotes:
      "Professional photos, clean card design with hover effects",
  },
  Testimonials: {
    ...
    testimonials: [
      {
        quote:
          "This platform transformed how we work. We have saved countless hours and improved productivity by 40%.",
        author: "Michael Chen",
      ...
      {
        quote:
          "The best investment we have made. The ROI was evident within the first month.",
      ...
      {
        quote:
          "Outstanding support and an incredibly intuitive platform. Highly recommended!",
    ...
    layoutDescription:
      "Carousel or grid of testimonial cards with quotes and attribution",
  ...
  Custom: {
    ...
    content:
      "This section can be customized to include any type of content that does not fit into the standard categories.",
...
export const componentContentPlanFactory = {
  build: (
    overrides: Override<typeof componentContentPlans.$inferInsert> = {}
  ) => {
    const allComponentTypes = Object.keys(COMPONENT_CONTENT_SEEDS);
    const componentType =
      overrides.componentType || faker.helpers.arrayElement(allComponentTypes);
    const seedData =
      COMPONENT_CONTENT_SEEDS[
        componentType as keyof typeof COMPONENT_CONTENT_SEEDS
      ] || {};
  ...
  async createWithOverview(
    ...
    const componentType =
      planOverrides.componentType || overviewOverrides.componentType || "Hero";
    const overview = await componentOverviewFactory.create({
      ...overviewOverrides,
      componentType,
    });
  ...
  async createFullPageContentPlans(
    pageId: number,
    websiteId: number,
...
export const themeFactory = {
  build: (overrides: Override<typeof themes.$inferInsert> = {}) => ({
    ...
    theme: {
      borderRadius: faker.helpers.arrayElement([
        "none",
        "sm",
        "md",
        "lg",
  ...
  async create(
    overrides: Override<typeof themes.$inferInsert> & { labels?: string[] } = {}
  ) {
    ...
    if (labels && labels.length > 0) {
      ...
      if (labelRecords.length > 0) {
        await db.insert(themesToThemeLabels).values(
...
export const templateFactory = {
  ...
  async createWithFiles(
    ...
    if (fileCount) {
      await templateFilesFactory.createMany(fileCount, {
...
export const templateFilesFactory = {
  build: (
    overrides: Override<typeof templateFiles.$inferInsert> = {},
    seedIndex?: number
  ) => {
    // If seedIndex is provided, use that specific seed data
    const seedData =
      seedIndex !== undefined ? templateFilesData[seedIndex] : null;
    ...
    if (seedData) {
      return {
        ...
        path: seedData.path,
        content:
          typeof seedData.content === "object"
  ...
  async createMany(
    ...
    const results = await db.insert(templateFiles).values(values).returning();
    return results;
  },
  ...
  async createAllForTemplate(templateId: number) {
    ...
    const values = templateFilesData.map((seedFile: any, index: number) => {
      return {
        ...
        path: seedFile.path,
        content:
          typeof seedFile.content === "object"
...
export const componentFactory = {
  ...
  async create(overrides: Override<typeof components.$inferInsert> = {}) {
    ...
    if (!fileSpecificationId) {
      const componentType =
        overrides.componentType || this.build().componentType;
      const fileSpec = await fileSpecFactory.create({ componentType });
      fileSpecificationId = Number(fileSpec.id);
    }
    ...
    const [component] = await db
      .insert(components)
      .values(
        this.build({ ...overrides, websiteId, pageId, fileSpecificationId })
      )
      .returning();
    return component;
  },
  ...
  async createWithContentPlan(
    ...
    contentPlanOverrides: Override<
      typeof componentContentPlans.$inferInsert
    > = {}
  ) {
    // First create the content plan
    const contentPlan = await componentContentPlanFactory.create(
      contentPlanOverrides
    );
  ...
  async createMany(
    count: number,
    overrides: Override<typeof components.$inferInsert> = {}
  ) {
    ...
    const results = await db.insert(components).values(values).returning();
    return results;
  },
};
...
export const taskFactory = {
  build: (overrides: Override<typeof tasks.$inferInsert> = {}) => ({
    ...
    subtype:
      overrides.subtype ||
      faker.helpers.arrayElement(["create", "update", "refactor", "fix"]),
  ...
  async create(overrides: Override<typeof tasks.$inferInsert> = {}) {
    ...
    const [task] = await db
      .insert(tasks)
      .values(
        this.build({ ...overrides, projectId, websiteId, fileSpecificationId })
      )
      .returning();
    return task;
  },
  ...
  async createWithFileSpec(
    ...
    const task = await this.create({
      ...
      fileSpecificationId: Number(fileSpec.id),
      title:
        taskOverrides.title || `Create ${fileSpec.componentType} component`,
  ...
  async createMany(
    count: number,
    overrides: Override<typeof tasks.$inferInsert> = {}
  ) {
    let projectId = overrides.projectId;
    ...
    const results = await db.insert(tasks).values(values).returning();
    return results;
  },
};
...
export const createComponentCodeTaskFactory = {
  ...
  async createWithMinimalDeps(
    componentType?: string,
    overrides: Override<typeof tasks.$inferInsert> = {}
  ) {
    const selectedType =
      componentType || faker.helpers.arrayElement(this.componentTypes);
...
export const codeTaskFactory = {
  ...
  async create(overrides: Override<typeof tasks.$inferInsert> = {}) {
    ...
    const taskData = this.build({
      ...
      title:
        overrides.title ||
        `${
          overrides.subtype === "create" || !overrides.subtype
            ? "Create"
            : "Update"
  ...
  async createWithFullStack(
    ...
    const { component, contentPlan } =
      await componentFactory.createWithContentPlan(
        {
          websiteId: Number(website.id),
          pageId: Number(page.id),
          name: selectedType,
...
export const websiteFilesFactory = {
  ...
  async createMany(
    count: number,
    overrides: Override<typeof websiteFiles.$inferInsert> = {}
  ) {
    let websiteId = overrides.websiteId;
    ...
    const results = await db.insert(websiteFiles).values(values).returning();
    return results;
  },
};
