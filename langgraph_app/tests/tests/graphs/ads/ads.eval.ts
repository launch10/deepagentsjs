...
type AdsEvalOutput = 
    | { type: 'followsInstructions'; originalHeadlines: string[]; newHeadlines: string[]; userRequest: string }
...
const testInstructionFollowing = async (input: { projectUUID: UUIDType; userRequest: string }): Promise<AdsEvalOutput> => {
...
const testQuestionAnswering = async (input: { projectUUID: UUIDType; question: string }): Promise<AdsEvalOutput> => {
    const result = await testGraph<AdsGraphState>()
        .withGraph(adsGraph)
        .withState({
            projectUUID: input.projectUUID,
