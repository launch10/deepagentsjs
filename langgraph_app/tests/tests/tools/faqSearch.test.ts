...
describe("FAQ Search Tool", () => {
  ...
  describe("Database Integration", () => {
    it("should have FAQ documents in the database from snapshot", async () => {
      ...
      const faqDocs = storedDocs.filter(
        (doc) => doc.tags && (doc.tags as string[]).includes("ads")
      );
  ...
  describe("Semantic Search Capabilities", () => {
    it("should find relevant FAQs for headline questions", async () => {
      ...
      if (results.length > 0) {
        const hasHeadlineRelated = results.some(
          (r) =>
    ...
    it("should find relevant FAQs for keyword questions", async () => {
      ...
      if (results.length > 0) {
        const hasKeywordRelated = results.some(
          (r) =>
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle very long queries", async () => {
      const longQuery =
        "I want to understand how to write the best headlines for my Google Ads campaign that will attract customers and improve my click-through rate while staying within character limits";
      const faqService = getFAQSearchService();
