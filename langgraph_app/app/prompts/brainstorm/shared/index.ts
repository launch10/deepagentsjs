...
export const backgroundPrompt = async (
...
export const whereWeArePrompt = async (
  state: BrainstormGraphState,
  config?: LangGraphRunnableConfig
) => {
  return `
...
export const collectedAnswersPrompt = async (
  state: BrainstormGraphState,
  config?: LangGraphRunnableConfig
) => {
...
export const currentTopicPrompt = async (
  state: BrainstormGraphState,
  config?: LangGraphRunnableConfig
) => {
  return `
...
export const remainingTopicsPrompt = async (
  state: BrainstormGraphState,
  config?: LangGraphRunnableConfig
) => {
  return `
...
export const getTopicDescriptions = async (topics: Brainstorm.TopicName[]): Promise<string> => {
  return await Promise.all(
    topics.map((topic) =>
      fs.promises.readFile(path.join(__dirname, `../topics/${topic}.md`), "utf-8")
    )
...
export const getHelpTemplates = async (topics: Brainstorm.TopicName[]): Promise<string> => {
  return await Promise.all(
    topics.map((topic) =>
      fs.promises.readFile(path.join(__dirname, `../help/${topic}.md`), "utf-8")
    )
...
export const getMarketingReferences = async (): Promise<string> => {
  ...
  return await Promise.all(files.map((file) => fs.promises.readFile(file, "utf-8"))).then(
...
const _sortedTopics = (topics: Brainstorm.Topic[]) => {
  return topics.sort(
    (a, b) =>
      Brainstorm.BrainstormTopics.indexOf(a.name) - Brainstorm.BrainstormTopics.indexOf(b.name)
  );
};
...
const _remainingTopics = (topics: Brainstorm.TopicName[]) => {
  ...
  return _sortedTopics(selectedTopics)
