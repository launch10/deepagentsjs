...
const renderArrayAsXML = (
  items: any[],
  itemTag: string,
  sortOption: "alphabetical" | "none" = "alphabetical"
): string => {
...
export const toXML = ({
  ...
  if (Array.isArray(cleanedValues)) {
    ...
    const finalItemTag =
      itemTag || (tag ? (tag.endsWith("s") ? tag.slice(0, -1) : `${tag}-item`) : "item");
  ...
  const sortedKeys =
    sortOption === "alphabetical" ? Object.keys(cleanedValues).sort() : Object.keys(cleanedValues);
