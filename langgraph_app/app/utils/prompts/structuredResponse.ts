...
export const schemaWithoutForeignKeys = (schema: z.ZodSchema): z.ZodSchema => {
  if (schema instanceof z.ZodUnion) {
    const cleanedOptions = schema.options.map((option: z.ZodSchema) =>
      schemaWithoutForeignKeys(option)
    );
  ...
  return schema.omit({
    ...
    ...Object.fromEntries(
      Object.entries(schema.shape).filter(([key]) => keyLooksLikeForeignKey(key))
    ),
