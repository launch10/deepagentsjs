...
export class SearchIconsService {
        ...
        iconNames.map(async ([name, filename]) => {
          try {
            ...
            const metadata = await fs.promises.readFile(
              path.resolve("./.data/lucide/icons", `${filename}.json`),
              "utf-8"
            );
  ...
  async generateEmbeddings(): Promise<any> {
    try {
      ...
      const newIconsToProcess = allIconEntries.filter(
        ([name, _metadata]) => !existingKeys.has(name)
      );
      ...
      for (let i = 0; i < newIconsToProcess.length; i += batchSize) {
        ...
        const batchItems = await Promise.all(
          batch.map(async ([name, metadata]) => {
            ...
            const llm = await getLLM({ skill: "writing" });
  ...
  async searchIcons(queries: string[], topK = 5): Promise<IconSearchResults> {
    try {
      ...
      queries.forEach((query, index) => {
        const normalizedQuery = normalizedQueries[index];
        if (normalizedQuery) {
          originalQueryMap.set(normalizedQuery, query);
        }
      });
  ...
  async suggestAlternatives(query: string, threshold = 0.5, topK = 5): Promise<IconResult[]> {
    try {
      ...
      const matches = results.filter((match) => match.similarity >= threshold).slice(0, topK);
