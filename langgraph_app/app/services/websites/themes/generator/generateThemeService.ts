...
import { HexToCssHslService } from "./hexToCssHslService";
import { 
  parseInput, 
  hexToColorInfo, 
...
export class GenerateThemeService {
  public execute(input: string | object | string[]): ServiceResponse {
    ...
    if (Array.isArray(input)) {
      ...
      const validationResult = this.validateAndProcessColors(inputTheme.colors);
    ...
    baseColorMap.forEach((bgColorHex, bgVarName) => {
      ...
      const hexToHsl = new HexToCssHslService();

      if (isStandardBackground) {
        finalCssVars[bgVarName] = hexToHsl.execute(bgColorHex); 
      ...
      if (isStandardBackground) {
        ...
        if (!bgColorInfo) {
          ...
          finalCssVars[fgVarName] = hexToHsl.execute(CONTRAST_COLORS.DARK); // Fallback
          finalCssVars[fgMutedVarName] = hexToHsl.execute(CONTRAST_COLORS.DARK); // Fallback
        ...
        if (standardFgInfo) {
          ...
          finalCssVars[fgVarName] = hexToHsl.execute(standardFgHex);
          ...
          finalCssVars[fgVarName] = hexToHsl.execute(standardFgHex); // Use fallback DARK
          warnings.push({
            type: 'contrast',
        ...
        if (actualStandardFgInfo) {
          ...
          if (mutedFgInfo) {
            finalCssVars[fgMutedVarName] = hexToHsl.execute(mutedFgInfo.hex);
            ...
            finalCssVars[fgMutedVarName] = hexToHsl.execute(standardFgHex); // Fallback to standard FG for muted
            warnings.push({
              type: 'contrast',
          ...
          finalCssVars[fgMutedVarName] = hexToHsl.execute(CONTRAST_COLORS.DARK); // Ultimate fallback
        }
      }
    });
  ...
  validateAndProcessColors(colors: string[]): ColorInfo[] | ErrorResponse {
