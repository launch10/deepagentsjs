...
interface NotifyStartProps {
    messages: BaseMessage[];
}

export class NotifyStartService {
    async execute(input: NotifyStartProps, config?: LangGraphRunnableConfig): Promise<NotifyStartOutputType> {
      ...
      const userRequest = lastHumanMessage(input);
