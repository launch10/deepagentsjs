...
export class ScenarioSaver {
  ...
  private async generateModifications(): Promise<ModificationEntry[]> {
    ...
    for (const file of currentFiles) {
      ...
      const fileMods = this.diffToModifications(
        originalContent || '',
        currentContent,
        file.path
      );
 ...
 */
export async function loadScenarioConfig(websiteName: string, scenarioName: string): Promise<ScenarioConfig | null> {
 ...
 */
export async function loadScenarioErrors(websiteName: string, scenarioName: string): Promise<ScenarioErrors | null> {
  const errorsPath = join(process.cwd(), 'tests', 'scenarios', websiteName, scenarioName, 'errors.json');
 ...
 */
export async function loadScenarioModifications(websiteName: string, scenarioName: string): Promise<ModificationEntry[]> {
  const modificationsDir = join(process.cwd(), 'tests', 'scenarios', websiteName, scenarioName, 'modifications');
