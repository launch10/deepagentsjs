import { pgTable, index, foreignKey, bigserial, varchar, text, jsonb, timestamp, bigint, boolean, uniqueIndex, integer, vector, date, check, numeric, serial, uuid, doublePrecision, primaryKey, pgView, pgSequence , customType} from "drizzle-orm/pg-core"
...
export const leads = pgTable("leads", {
	...
	index("index_leads_on_project_id").using("btree", table.projectId.asc().nullsLast().op("int8_ops")),
	uniqueIndex("index_leads_on_project_id_and_email").using("btree", table.projectId.asc().nullsLast().op("int8_ops"), table.email.asc().nullsLast().op("text_ops")),
]);
