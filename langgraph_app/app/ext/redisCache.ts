...
export class RedisCache<V = unknown> extends BaseCache<V> {
  ...
  async query(pattern?: string): Promise<string[]> {
    // Default to querying all keys under the base namespace
    const searchPattern = pattern || `${this.namespace.join(",")}*`;
    const keys: string[] = [];
  ...
  async clear(namespaces: CacheNamespace[] = []): Promise<void> {
    ...
    for (const namespace of namespaces) {
      // If empty namespace, clear all keys under the base namespace
      const targetNamespace = namespace.length === 0 ? this.namespace : namespace;
      const strNamespace = targetNamespace.join(",");
      
      // Pattern to match all keys that start with the namespace
      // Using wildcard to match both "namespace:*" and "namespace,*" patterns
